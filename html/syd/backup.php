...
session_start();
	...
	if ($result->num_rows > 0) {
	  ...
	  while($row = $result->fetch_assoc()) {
		$status = $row["status"] === 0 ? "Stop!!!" : "Sending";
...
console.log(response);
	...
	console.log(response);
								...
								$("#table_body").prepend(response.data.new_row);
	                        },
			...
			$("#sendBtn").click(function (e) {
				...
				sim = setInterval(progressSim, 8000);
			});
					...
					$.ajax('ajax.php',
			                    {
		        	                type: 'POST',
			                        enctype: 'multipart/form-data',
			                        data: {action : "send" ,number : numbers[start - 1] , message : msg , token : token},
			                        dataType: 'json',
			                        success: function (response, status, xhr) {
console.log(response);
							if(response.stop){
